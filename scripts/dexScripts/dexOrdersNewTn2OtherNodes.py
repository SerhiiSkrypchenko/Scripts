...
vaults1 = ([vault21, vault22])
vaults2 = ([vault23, vault24])


def DexTransactions(url1, url2, sleepTime):
    k = 1
    while True:
        ...
        for i in range(1, 7):
            ...
            buyDex(url1, vaults1, str(pairRate), str(offerAmount))
            time.sleep(sleepTime)

            print(" --------------- NEW SELL STARTED ------------------ LOW-HIGH ----------------- pair rate * offerAmount = " + str(pairRate*offerAmount))
            sellDex(url2, vaults2, str(pairRate), str(offerAmount))

            """order = [buyDex(url, vaults, str(pairRate), str(offerAmount)), sellDex(url, vaults, str(pairRate), str(offerAmount))]
            random.choice(order)
            time.sleep(4)
            if order == buyDex(url, vaults, str(pairRate)):
                sellDex(url, vaults, str(pairRate), str(offerAmount))
            else:
                buyDex(url, vaults, str(pairRate))"""
        ...
        for j in range(1, 7):
            ...
            print(
                " --------------- NEW BUY STARTED ------------------ HIGH-LOW ----------------- pair rate * offerAmount = " + str(
                    pairRate * offerAmount))
            buyDex(url2, vaults2, str(pairRate), str(offerAmount))
            time.sleep(sleepTime)
            print(" --------------- NEW SELL STARTED ------------------- HIGH-LOW ----------------- pair rate * offerAmount = " + str(pairRate*offerAmount))
            sellDex(url1, vaults1, str(pairRate), str(offerAmount))
            time.sleep(sleepTime)
            j += 1
        time.sleep(900)
...
def sellDex(url, vaults, pairRate, offerAmount):
    for i in range(0, len(vaults)):
        ...
        try:
            print(" <-------- START SELL ORDER ----------> ")
            response = requests.request("POST", "http://" + url + "/rest/dex/offer", data=payload,
                                    headers=headers)
            print("RESPONSE ETH = " + str(response.json()))
            time.sleep(8)
            response1 = requests.request("POST", "http://" + url + "/rest/dex/offer", data=payload1,
                                        headers=headers)
...
DexTransactions(testNet2.localhost, testNet2.peer1, 180)